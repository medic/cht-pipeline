...
FROM
  (
    ...
    */
    ...
    WHERE NOT EXISTS (SELECT NULL FROM get_muted_contacts(to_date,'person') muted
  ...
  LEFT JOIN
    ...
    (
      ...
      WHERE
        type = 'clinic'
        AND NOT EXISTS (SELECT NULL FROM get_muted_contacts(to_date,'clinic') muted
  ...
  (
    ...
    WHERE
      cm.type = 'clinic' AND
      NOT EXISTS (SELECT NULL FROM get_muted_contacts(to_date,'clinic') muted WHERE muted.contact_uuid = visit.place_id)
    GROUP BY
      CHW_UUID,
      interval_number
