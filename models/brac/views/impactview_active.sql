    ...
    sum(
        CASE
            WHEN ((EXISTS ( SELECT 1
               FROM {{ ref("form_metadata") }} AS meta
              WHERE ((meta.chw = chw_facility.chw_uuid) AND ((date_trunc('month'::text, meta.reported))::date = month_facility.month)))) OR (EXISTS ( SELECT 1
               FROM (({{ ref("contactview_metadata") }} AS person
                 JOIN {{ ref("contactview_metadata") }} AS family ON ((person.parent_uuid = family.uuid)))
                 JOIN {{ ref("contactview_metadata") }} AS chw_area ON ((family.parent_uuid = chw_area.uuid)))
            ...
            ELSE 0
        END) AS active_chws
FROM ({{ ref("impactview_month_facility") }} AS month_facility
    LEFT JOIN {{ ref("impactview_chw_facility") }} AS chw_facility ON ((month_facility.facility_join_field = chw_facility.facility_join_field)))
